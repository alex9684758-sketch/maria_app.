...
class _CentroDeMandoState extends State<CentroDeMando> {
  ...
  void _intentarDesbloqueo() {
    if (_codigoController.text == "LIBERTAD2026") {
      ...
      _mostrarMensaje("Código Inválido. Contacta al Arquitecto.");
    }
  }
  ...
  // --- MODIFICACIÓN LEGAL: FUNCIÓN PARA MOSTRAR AVISO ---
  void _mostrarAvisoLegal() {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text("AVISO DE RESPONSABILIDAD", style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.bold)),
        content: const SingleChildScrollView(
          child: Text(
            "1. El sistema MARÍA es una herramienta de aprendizaje y autogestión.\n\n"
            "2. El contenido sobre activos e inversión es puramente educativo. No constituye asesoría financiera legal.\n\n"
            "3. El usuario asume total responsabilidad por el uso de la información aquí contenida.\n\n"
            "4. El Arquitecto no se hace responsable por pérdidas, decisiones de terceros o el mal uso de la tecnología IA integrada.\n\n"
            "5. Al usar esta plataforma, aceptas la soberanía individual como eje rector.",
            style: TextStyle(fontSize: 13, height: 1.5),
          ),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text("ENTENDIDO Y ACEPTO")),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.gavel_rounded, color: Colors.blueGrey),
            onPressed: _mostrarAvisoLegal, // Acceso rápido legal
          )
        ],
      ),
            ...
            Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 10),
                  // Link legal en el footer
                  TextButton(
                    onPressed: _mostrarAvisoLegal,
                    child: const Text("TÉRMINOS DE SOBERANÍA Y LEGALIDAD", 
                      style: TextStyle(fontSize: 10, decoration: TextDecoration.underline, color: Colors.blueGrey)),
                  ),
                ],
              ),
            ),
  ...
  // Los widgets auxiliares (_buildFaseCard, _mostrarDetalleFase, etc.) se mantienen igual...
